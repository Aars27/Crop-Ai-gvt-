...
import '../widget/Constants.dart'; // Add this import for input formatters
...
/*  final TextEditingController _majorMaintenanceController = TextEditingController();*/
  ...
  String? _selectedManPowerRoll = 'Select Man Power'.tr();
...
/*
  ...
  List<String> _soilConditionName = ['Select Soil Condition'.tr()];
  String? _selectedSoilCondition = 'Select Soil Condition'.tr();
...
final List<Map<String, dynamic>> _manpowerTypes = [];
  ...
  final List<String> _manPowerRoll = ['Select ManPower'];
  ...
  Future<void> _submitForm() async {
    if (_formKey.currentState!.validate()) {
      ...
      try {
        ...
        if (_selectedSoilCondition != null &&
            _selectedSoilCondition != 'Select Soil Condition'.tr()) {
    ...
    if (selectedCategories.isNotEmpty) {
      ...
      if (availableCategories != null) {
        ...
        final Map<String, dynamic> requestData = {
          ...
          'machine_ids': machineIds, // Now as List<String>
          'tractor_ids': tractorIds, // Now as List<String>
...
print("Final manpower_categories: ${jsonEncode(manpowerCategories)}");
        ...
        // Make API call
        final response = await http.post(
          Uri.parse('${Constanst().base_url}land-preparation-store'),
          headers: {
            'Authorization': 'Bearer $token',
            'Content-Type': 'application/json',
  ...
  Future _fetchSoilConditions() async {
    try {
      ...
      final response = await http.post(
        Uri.parse('${Constanst().base_url}soil-condion'),
        headers: {
          'Authorization': 'Bearer $token',
          'Accept': 'application/json',
        },
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['status'] == 'success' &&
          ...
          setState(() {
            ...
            _soilConditionName = ['Select Soil Condition'.tr()];
            _soilConditionName.addAll(_soilConditions
                .map((item) => item['soil_condition'].toString())
                .toList());
          });
        }
      } else {
