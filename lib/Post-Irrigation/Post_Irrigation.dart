...
// Add these variables at the top with other declarations
  ...
  TimeOfDay? _endTime = TimeOfDay.fromDateTime(DateTime.now().add(
          const Duration(hours: 4)) // Initialize with current time + 4 hours
      );
...
//  List<String> _manPowerName = ['Select Man Power'];
  ...
  List<String> _irrigation = ['Select Irrigation'.tr()];
  List<Map<String, dynamic>> _irrigationData = [];

  String? _selectedSourceWater;
  List<String> _water = ['Select Water Source'.tr()];
  List<Map<String, dynamic>> _watersourceData = [];

  String? _selectedCapacity;
  List<String> _capacity = ['Select Capacity'.tr()];
...
// Update area based on selected plot
  ...
  Future<void> _selectDate(BuildContext context, bool isStartDate) async {
    final DateTime? picked = await showDatePicker(
      context: context,
      initialDate:
          isStartDate ? (_startDate ?? DateTime.now()) : DateTime.now(),
...
// Get manpower type ID
        ...
        final Map<String, dynamic> requestData = {
          ...
          'day_ofter_swowing':
              formattedSowingDate, // Use the formatted sowing date
        ...
        // Make API call
        final response = await http.post(
          Uri.parse('${Constanst().base_url}post-irrigation'),
          headers: {
            'Authorization': 'Bearer $token',
            'Content-Type': 'application/json',
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  void _resetForm() {
    setState(() {
      ...
      _selectedIrrigationType = 'Select Type'.tr();
      _selectedSourceWater = 'Select Source Water'.tr();
      _selectedCapacity = 'Select Capacity'.tr();
      _selectedLandQuality = 'Select Land Quality'.tr();
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['status'] == 'success') {
          ...
          setState(() {
            ...
            _irrigation = ['Select Irrigation'.tr()];
      ...
      if (response.statusCode == 200) {
        ...
        if (responseData['status'] == 'success') {
          ...
          setState(() {
            ...
            _selectedSourceWater = 'Select Water Source'.tr();
          });
        } else {
          throw Exception('API returned false success status');
        }
